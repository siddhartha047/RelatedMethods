...
def train(args, model, device, data, optimizer, epoch, mask=None):
    ...
    if args.data in ['dummy']: #['ogbn-proteins']:
...
def evaluate_ogb(args, model, device, data, evaluator, is_test_set=False):
    ...
    with torch.no_grad():
        ...
        elif args.data in ['dummy']: #['ogbn-proteins']:
...
def main():
    ...
    for i in range(args.iters):
        ...
        data.edge_attr = torch.ones(data.num_edges)        
        print(data.x.shape)
        print(data.y.shape)
        print(data)
        ...
        sid_f1s.append(test_acc)

        results_to_file(args, train_acc, val_acc, test_acc, train_time_total, time.time()- t_test_0)

    print(args.data)
    print(f'Mean Std of Test F1 Score: {100*np.mean(sid_f1s):.2f} ± {100*np.std(sid_f1s):.2f}')
