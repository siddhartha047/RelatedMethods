    ...
    with torch.no_grad():
        for batch in cluster_loader:
            ...
            for i in range(args.num_samples_eval):                    
                ...
                if args.calc_homophily and sampled_edge_index is not None:
                    n_homo.append(homophily(sampled_edge_index, batch.y, method='node'))
                    e_homo.append(homophily(sampled_edge_index, batch.y, method='edge'))
...
#             num_test_examples += batch.test_mask.sum().item()
    ...
    if args.calc_homophily:
        print(f'Hn {np.mean(n_homo):.4f} He {np.mean(e_homo):.4f}')    
