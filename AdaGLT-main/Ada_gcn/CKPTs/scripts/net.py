...
class net_gcn_dense(nn.Module):
    def __init__(self, embedding_dim, edge_index, device, spar_wei, spar_adj, num_nodes, use_res, use_bn, coef=None, mode="prune"):
        ...
        self.adj_binary = to_dense_adj(edge_index, max_num_nodes=self.num_nodes)[0]
    ...
    def learn_soft_edge(self, x, edge_index, ln=0):
        ...
        variance = edge_weight.var(unbiased=False)
        if edge_weight.numel() <= 1 or float(variance.detach().cpu()) < 1e-8:
            return torch.ones_like(edge_weight)
        edge_weight = ((edge_weight - mean) * torch.sqrt(0.0001 / variance)) + 1
        edge_weight = torch.nan_to_num(edge_weight, nan=1.0, posinf=1.0, neginf=1.0)
        return edge_weight
    ...
    def adj_pruning2(self, adj, thres, prev_mask, tau=0.1, val_test=False):
        ...
        edge_weight = torch.nan_to_num(edge_weight, nan=0.0, posinf=0.0, neginf=0.0)
        adj = to_dense_adj(edge_index, edge_attr=edge_weight, max_num_nodes=self.num_nodes)[0]
        B = adj.size(0)
        thres_trans = lambda x: self.coef*(torch.pow(x,3) + 20*x)     
        y_soft = torch.sigmoid((adj - thres_trans(thres)) / tau)
        y_soft = torch.nan_to_num(y_soft, nan=0.0, posinf=1.0, neginf=0.0)
        # y_hrad = (y_soft > 0.5).float()
        y_hrad = ((y_soft + torch.eye(adj.shape[0]).to(self.device))  > 0.5).float()
        ret = y_hrad - y_soft.detach() + y_soft
        ret = ret * prev_mask if prev_mask is not None else ret
        return torch.nan_to_num(ret, nan=0.0, posinf=1.0, neginf=0.0)
