...
from EDSparseDataset import infer_embedding_dim, load_dense_tensors
...
def torch_normalize_adj(adj, device):
    # AdaGLT's native citation loader uses AugNormAdj, so every node has a
    # self-loop. The EDSparse connector supplies the original edge set so
    # sparsity is measured against original directed edges; add only missing
    # loops here, after the learned edge mask has been applied.
    adj = adj.clone()
    node_index = torch.arange(adj.shape[0], device=adj.device)
    diagonal = adj[node_index, node_index]
    adj[node_index, node_index] = torch.where(
        diagonal != 0,
        diagonal,
        torch.ones_like(diagonal),
    )
    rowsum = adj.sum(1)
    safe_rowsum = rowsum.clamp_min(1e-12)
    d_inv_sqrt = torch.pow(safe_rowsum, -0.5).flatten()
    d_inv_sqrt = torch.where(rowsum.flatten() > 0, d_inv_sqrt, torch.zeros_like(d_inv_sqrt))
    d_inv_sqrt = torch.nan_to_num(d_inv_sqrt, nan=0.0, posinf=0.0, neginf=0.0)
    d_mat_inv_sqrt = torch.diag(d_inv_sqrt).to(device)
    normalized = adj.mm(d_mat_inv_sqrt).t().mm(d_mat_inv_sqrt)
    return torch.nan_to_num(normalized, nan=0.0, posinf=0.0, neginf=0.0)


def torch_row_normalize_features(features):
    """Match AdaGLT's native citation feature preprocessing."""
...
def rewind_compatible_weights(model, rewind_weight):
    """Restore parameters shared by the pruning and retained GCNs."""

    if rewind_weight is None:
        raise ValueError("AdaGLT retained training requires captured rewind weights")
    retained_state = model.state_dict()
    compatible = {
        key: value
        for key, value in rewind_weight.items()
        if key in retained_state and retained_state[key].shape == value.shape
    }
    if not compatible:
        raise RuntimeError("No compatible AdaGLT parameters were available for rewinding")
    model.load_state_dict(compatible, strict=False)
    print(
        "[Rewind] restored "
        f"{len(compatible)}/{len(retained_state)} retained-model tensors"
    )
...
def load_citation(dataset_str="cora", normalization="AugNormAdj", porting_to_torch=True,data_path=datadir, task_type="semi"):
    ...
    if data_path == datadir:
        ...
        return adj.to_sparse().float(), features.float(), labels.long(), idx_train.long(), idx_val.long(), idx_test.long(), degree, learning_type
...
def calcu_sparsity(edge_masks, edge_num):
    if edge_masks is None or edge_num <= 0:
        return 0
    with torch.no_grad():
        spar_ls = []
        for ln in range(len(edge_masks)):
            mask = torch.nan_to_num(edge_masks[ln], nan=0.0, posinf=1.0, neginf=0.0)
            spar = (1 - (mask.sum().item() / edge_num)) * 100
            spar_ls.append(spar)
        if not spar_ls:
            return 0
