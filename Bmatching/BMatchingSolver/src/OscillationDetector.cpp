...
bool OscillationDetector::checkState(double *a, double *b) {
	double * A = new double[2 * length];
	memcpy(A, a, length * sizeof(double));
	memcpy(A + length, b, length * sizeof(double));
...
int OscillationDetector::hash(double * a) {
...
bool OscillationDetector::insert(int checkSum, double * a) {
	int i = quadraticProbe((checkSum % capacity + capacity) % capacity, a);
	if (vectors[i])
		return false;
	else {
		vectors[i] = new double[2 * length];
		memcpy(vectors[i], a, 2 * length * sizeof(double));
		size++;
		if (size > capacity / 2) {
...
int OscillationDetector::quadraticProbe(int index, double * a) {
	int i = index, c = 0;
	while(vectors[i]) {
		bool equal = true;
