...
int configuredThreadCount() {
	const char * configured = std::getenv("BMATCH_NUM_THREADS");
	if (configured) {
...
BeliefPropagator::BeliefPropagator() {
	...
	lookups = 0;
	detector = 0;
	numThreads = configuredThreadCount();
	alpha = 0;
	beta = 0;
	dontcare = 0;
	betaIndex = 0;
...
void BeliefPropagator::updateBeliefs() {
	...
	double * newAlpha = new double[size];
	double * newBeta = new double[size];

	const int workerCount = std::min(numThreads, size);
	if (workerCount > 1) {
		// initialize threads
	...
	double * betaCopy = new double[size];
	for (int i = 0; i < size; i++) {
		betaIndex[i] = i;
		betaCopy[i] = beta[i];
	}
...
int BeliefPropagator::updateScore(double score, double *bestScores, int *bestIndex,
...
void BeliefPropagator::updateRow(double * newAlpha, double * newBeta, int i) {
	...
	double * bestScores = heap->getDoubleArray();
