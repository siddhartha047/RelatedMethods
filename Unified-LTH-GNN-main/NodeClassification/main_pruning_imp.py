...
if __name__ == "__main__":
    ...
    for p in range(2):
        ...
        glt_sparsities.append(adj_spar)
        glt_test_accs.append(final_acc_test)

    print(f'{np.mean(glt_test_accs)*100:.2f} ± {np.std(glt_test_accs)*100:.2f}')
    print(f'{np.mean(glt_sparsities):.2f} ± {np.std(glt_sparsities):.2f}')
