...
def maybe_sparsfication(data, dataset, follow_by_subgraph_sampling, random=False, is_undirected=True, reweighted=True, target_ratio=None):
    ...
    # Q is DSpar's sample budget.  When a common kept-edge budget is supplied
    # by the baseline wrapper, use it as the edge-sample budget; otherwise keep
    # the repository's original 0.5 * N setting.
    if target_ratio is None:
        Q = int(0.5*N) # same settings as SGS-GNN
    else:
