...
parser.add_argument('--root', type=str, default='/scratch/gilbreth/das90/dspar')
...
parser.add_argument('--kept_ratio', type=float, default=None,
                    help='Optional common kept-edge sample budget for sparsification wrappers.')
...
def compute_macro_f1(logits, y, mask=None) -> float:
    ...
    if y.dim() > 1:
        y = y.argmax(dim=-1)
    return macro_f1_percent(y, logits) / 100.0
...
def main():
    ...
    data, num_features, num_classes = my_get_data(args.root, args.dataset, False, enable_sparsify, args.random_sparsify, args.kept_ratio)
