...
import torch.backends.cudnn as cudnnxw
...
import models
from data import get_data, my_get_data
...
def main():
    ...
    #data, num_features, num_classes = get_data(args.root, args.dataset, True, enable_sparsify, args.random_sparsify)
    data, num_features, num_classes = my_get_data(args.root, args.dataset, True, enable_sparsify, args.random_sparsify)
    sampler_data = data
    ...
    if model_config['name'] == 'graph_saint':
        ...
        num_parts = int(np.ceil(num_edges / num_edges_per_partition))    
    ...
    if args.debug_mem:
        ...
        print(f'training epoch/s: {1/(np.sum(epoch_per_sec)) }')
        exit()
