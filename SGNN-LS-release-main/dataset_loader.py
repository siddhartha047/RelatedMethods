import argparse
import torch
from torch_geometric.datasets import Planetoid, WebKB, WikipediaNetwork, Amazon, Actor, Coauthor
...
def DataLoader(name):
    ...
    if name in ['cora', 'smallcora', 'citeseer', 'pubmed']:
        dataset_name = 'cora' if name == 'smallcora' else name
        ...
        dataset = Coauthor(root=root, name='CS', transform=T.NormalizeFeatures())
