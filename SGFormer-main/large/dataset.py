...
def load_dataset(data_dir, dataname, sub_dataname=''):
    ...
    """
    print(dataname)
    if dataname in SGS_PREPARED_DATASETS:
        return load_prepared_sgs_dataset(dataname)
...
def require_google_drive_downloader():
    if gdd is None:
        raise ImportError(
            "google_drive_downloader is required for this SGFormer dataset path. "
            "Use the prepared SGS datasets or install the original dependency in a separate environment."
        )


def require_ogb():
    if NodePropPredDataset is None or PygNodePropPredDataset is None:
        raise ImportError(
            "ogb is required for this SGFormer dataset path. "
            "Use the prepared SGS datasets or install the original dependency in a separate environment."
        )


def load_prepared_sgs_dataset(name):
    repo_root = Path(__file__).resolve().parents[1]
    payload_path = repo_root / 'data_saved' / f'{name}.pt'
    if not payload_path.exists():
        raise FileNotFoundError(
            f"Missing prepared SGS dataset at {payload_path}. "
            f"Run `python {repo_root / 'prepare_sgs_dataset.py'} --dataset {name}` first."
        )

    payload = torch.load(payload_path, map_location='cpu')
    dataset = NCDataset(name)
    node_feat = payload['x'].to(torch.float)
    edge_index = payload['edge_index'].to(torch.long)
    label = payload['y'].to(torch.long)
    ...
    def load_fixed_splits():
        return {
            'train': train_mask.nonzero(as_tuple=False).view(-1),
            'valid': val_mask.nonzero(as_tuple=False).view(-1),
            'test': test_mask.nonzero(as_tuple=False).view(-1),
        }

    dataset.load_fixed_splits = load_fixed_splits
    dataset.train_idx = train_mask.nonzero(as_tuple=False).view(-1)
    dataset.valid_idx = val_mask.nonzero(as_tuple=False).view(-1)
    dataset.test_idx = test_mask.nonzero(as_tuple=False).view(-1)
    return dataset
...
def load_pokec_mat(data_dir):
    ...
    if not path.exists(f'{data_dir}/pokec/pokec.mat'):
        require_google_drive_downloader()
