...
def parse_method(method, args, c, d, device):
    if method == 'gcn':
        ...
        if GPSModel is None:
            raise ImportError('graphgps dependencies are unavailable in the current environment')
        ...
        if Graphormer is None:
            raise ImportError('graphormer dependencies are unavailable in the current environment')
        ...
        if graphTrans is None:
            raise ImportError('graphtrans dependencies are unavailable in the current environment')
...
def parser_add_main_args(parser):
    ...
    parser.add_argument('--edge_keep_ratio', type=float, default=1.0,
                        help='fraction of off-diagonal input edges to retain before self-loops')
    parser.add_argument('--count_self_loops_in_budget', type=str, default='false',
                        help='whether self-loops count toward edge_keep_ratio budget')
