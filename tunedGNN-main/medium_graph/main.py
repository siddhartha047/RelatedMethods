...
if args.rand_split:
    ...
    split_idx_lst = fixed_split if isinstance(fixed_split, list) else [fixed_split]
...
for run in range(args.runs):
    if args.dataset.lower() in ('coauthor-cs', 'coauthor-physics', 'amazon-computer', 'amazon-photo', 'cora', 'citeseer', 'pubmed'):
    ...
    for epoch in range(args.epochs):
        ...
        out = result[-1]
        train_f1 = macro_f1_percent(dataset.label[split_idx['train']], out[split_idx['train']]) / 100.0
        test_f1 = macro_f1_percent(dataset.label[split_idx['test']], out[split_idx['test']]) / 100.0

        logger.add_result(run, (result[0], result[1], result[2], result[3], train_f1, test_f1))
