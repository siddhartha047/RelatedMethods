...
from CustomSpectralSelector import CustomSpectralSelector
from CustomSubmodularSelector import CustomSubmodularSelector


func=CustomSpectralSelector
...
def GetLocalEdgesParallel(org_u, u, nodes, features, K):
    ...
    model = func(K, initial_subset=[u], optimizer='stochastic')
    model.fit(features)    
...
def CustomGetLocalEdgesParallel(org_u, u, nodes, features, K, optimizer):
    ...
    model = func(K, initial_subset=[u], optimizer=optimizer)
    model.fit(features)    
...
def GetLocalEdgesFaciltyLocation(org_u, u, nodes, features, K, optimizer, metric):
    ...
    model = CustomSubmodularSelector(K, initial_subset=[u], metric=metric, optimizer=optimizer)
    model.fit(features)    
...
if __name__ == '__main__':  
    ...
    model = CustomSpectralSelector(3, optimizer='lazy')
    start = time.time()
    model.fit(X)
    print("Time:", time.time()-start)
    
   
    model = CustomSubmodularSelector(3, initial_subset=[0], metric='euclidean', optimizer='approximate-lazy')
    start = time.time()
    model.fit(X)
    print("Time:", time.time()-start)
